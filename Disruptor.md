...
Disruptor是数组实现的，对于遍历来说，链表的效率一定比数组低，对比ConcurrentLinkedQueue， JDK中没有ConcurrentArrayQueue，因为删除、添加的时候边界检查和扩容复制太麻烦，效率相当低。
而Disruptor的作者很聪明，想到了一个办法，就是用环形数组。Disruptor可以看作是用数组实现的ConcurrentArrayQueue，另外这个Queue还是首尾相连的。链表遍历慢是一个，他还要维护头尾指针，
往头部加的时候要加锁，从尾部删的时候也要加锁。环形的只维护一个位置sequence，序列。这个序列代表下一个有效的元素指在什么位置上，相当于他只有一个指针，来回转。加在哪个位置上？与数组length
取模（用位运算：12 % 8 = 12 & （8-1））。由于环形数组的总大小是固定的，已有的没消费的元素是有个标记的，生产者生产满了之后就不能在生产了，它有一个等待策略（共有8种，最常见的是blocking 
wait，阻塞在那里）

无锁，高并发，**单机**性能最高的MQ。使用环形Buffer，直接覆盖（不用清除）旧的数据，无首尾指针。降低GC频率。性能达到单线程600万订单/秒

简单理解就是一个内存里的用于存放元素的高效率的队列
...
1. 定义Event - 队列中需要处理的元素。环形队列里的每个元素都是事件

2. 定义Event工厂，用于填充队列

   > 这里牵扯到效率问题：disruptor初始化的时候，会调用Event工厂，对ringBuffer进行内存的提前分配, 每个位置先放入一个event，这个Event是new的还是读数据库产生的都可以，总而言之调用newInstance产生
   > 以后再往里放数据的时候就不new Event了，而是修改里面的值
   > GC产生频率会降低
